...
# HUD geometry
const PANEL_POS = Vector2(16, 52)
const PANEL_SIZE = Vector2(455, 320)
const HEADER_H = 38.0
const FOOTER_H = 59.0
...
var sensory_bin = -1
var neural_angle = 0.0
var neural_angle_diff = 0.0

var hud_note = -1
var hud_active_note = -1
var hud_lookahead = false
...
var panel_border = Color(0.00, 0.00, 0.00, 0.00)
var brain_bg = Color(0.015, 0.025, 0.034, 0.72)

var text_main = Color(1, 1, 1, 1)
var text_dim = Color(1, 1, 1, 1)
...
func _ready():
	...
	font_data.font_path = "res://assets/fonts/Consolas.ttf"

	hud_font = DynamicFont.new()
	hud_font.font_data = font_data
	hud_font.size = 13
	hud_font.outline_size = 1
	hud_font.outline_color = Color(0, 0, 0, 1)
...
func _append_segments(dst, src, stride):
	# PoolVector arrays are value/copy-on-write types in Godot 3.
	# Return the modified array explicitly instead of assuming the caller's
...
func _mesh_from_points(points):
	...
	# Godot 3 ArrayMesh expects ARRAY_VERTEX to be PoolVector3Array,
	# even when the mesh is rendered by MeshInstance2D.
	# The previous optimized HUD passed PoolVector2Array here, so the
	# text rendered but the neuron meshes were empty/invisible.
	var verts3 = PoolVector3Array()

	for p in points:
		verts3.append(Vector3(p.x, p.y, 0.0))

	var arrays = []
	arrays.resize(Mesh.ARRAY_MAX)
	arrays[Mesh.ARRAY_VERTEX] = verts3
...
func _compile_static_geometry():
	...
	print(
		"[FlyBrainMiniHUD] MBON mesh input: ",
		mbon_points.size(),
		" vertices / ",
		int(mbon_points.size() / 2),
		" lines"
	)
	...
	print(
		"[FlyBrainMiniHUD] Background mesh valid: ",
		background_instance.mesh != null
	)
...
func _font():
	if hud_font != null and hud_font.font_data != null:
		return hud_font
...
func _draw():
	...
	# ==========================================
	# CONTINUOUS NEURAL AIM
	# ==========================================

	if angle_valid:

		var aim_text = (
			"IN %.1f  BIN %d  NN %.1f  DIFF %.1f deg"
			% [
				target_angle,
				sensory_bin,
				neural_angle,
